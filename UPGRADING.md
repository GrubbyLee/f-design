...
Replace the public source folder with the new release, then run its `sync-aide.sh`. Do not copy `.codex/`, `.f-design/profile.md`, caches, or repository metadata into the source package.

## Compatibility

- Public preferences remain separate in `.f-design/profile.md` and `~/.f-design/preferences.md`.
- `f-design.json` is the machine-readable release manifest.
...
```bash
python3 scripts/f-design-doctor.py --json
```
